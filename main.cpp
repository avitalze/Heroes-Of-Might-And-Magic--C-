...
int main(int argc, char *argv[]){
	...
	if( argc==2){
		ifstream filename;
		filename.open("Debug/Game/gameDetails.txt");
		string gameDet[15];
		string line;
		int k=0;
		...
		for(n=0;n<realNumPlayer;n++){
			string filePath="Debug/HeroesInGame/"+gameDet[n+1]+"/Details.txt";
			ifstream playerDet;
		...
		for(m=0;m<realNumPlayer;m++){
			string fileToDelete="Debug/HeroesInGame/"+gameDet[m+1]+"/Details.txt";
			string folderToDelete="Debug/HeroesInGame/"+gameDet[m+1];
			remove(fileToDelete.c_str());
			remove(folderToDelete.c_str());
		}
		remove("Debug/HeroesInGame");
		remove("Debug/Game/gameDetails.txt");
		remove("Debug/Game");
		...
		struct stat myst;
		if ((stat("Debug/Game", &myst)==0) && S_ISDIR(myst.st_mode)){//HERO
			ifstream filename;
			filename.open("Debug/Game/gameDetails.txt");
			string array[15];
			string line;
			...
			if( (stat("Debug/HeroesInGame",&myst)==0) && S_ISDIR(myst.st_mode)){
				string first= array[0];
				int numOfOld= atoi(first.c_str());
				int m;
				for(m=0;m<numOfOld;m++){
					string fileToDelete="Debug/HeroesInGame/"+array[m+1]+"/Details.txt";
					string folderToDelete="Debug/HeroesInGame/"+array[m+1];
					remove(fileToDelete.c_str());
					remove(folderToDelete.c_str());
				}
			}
							...
							if(Players[index]->getTotalCreatures()==0){// defence not have army
								...
								while (!creatureOk){
									...
									try{
										...
										cout<< "no creature available" << endl;
									}
								}// while creatureOk
			...
			case 3:
				...
				case 1:
				{
					...
					try{
						...
						cout << "your buy not accepted" << endl;
						break;
					}
				}
				break;
				...
				case 7:
				{
					...
					mkdir("Debug/HeroesInGame", 0777);
					mkdir("Debug/Game", 0777);
					...
					for( i=0;i<numOfPlaying;i++){
						...
						folderPath="Debug/HeroesInGame/"+name;//Debug/HeroesInGame/revi
						mkdir(folderPath.c_str(), 0777);
						filePath=folderPath+"/Details.txt";//Debug/HeroesInGame/revi/Details.txt
					...
					ofstream gameDetails("Debug/Game/gameDetails.txt");
...
/*
	const int dir_err = mkdir("Debug/Heroes", 0777);

	mkdir("Debug/Heroes/Hero1", 0777);
	ofstream o("Debug/Heroes/Hero1/details.txt");
	...
	ifstream file ("Debug/Heroes/Hero1/details.txt");
